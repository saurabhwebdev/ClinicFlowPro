...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Create invoice
        if ($invoice->create()) {
            header('Location: index.php?message=' . urlencode('Invoice created successfully'));
            exit;
        } else {
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" name="notes" rows="4" 
                                          placeholder="Additional notes or payment instructions"></textarea>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="card">
    ...
    document.getElementById('patient_id').addEventListener('change', function() {
        ...
        fetch(`get_appointments.php?patient_id=${patientId}`)
            ...
            .then(data => {
                ...
                data.forEach(apt => {
                    options += `<option value="${apt.id}">${apt.date} ${apt.time} - ${apt.service}</option>`;
                });
